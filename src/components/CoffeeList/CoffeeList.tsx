...
import {deleteCoffee, deleteFromFavorites, addToSelected} from '../../utils/api'; // Імпорт функцій
              ...
              {coffees.map(coffee => (
                  <li key={coffee._id} className="container-item">
                    <div className="container-details">
                      ...
                      {coffee.capsuleMaterial &&
                        <span><strong>Capsule Material:</strong> {coffee.capsuleMaterial}<br/></span>}
                      ...
                      {favorite ? (
                          <div className="counter">
                            <button className="counter-button"
                                    onClick={() => handleCounterChange(coffee._id, 1)}>+
                            </button>
                            ...
                            <button className="add-coffee-button"
                                    onClick={() => handleAddCoffee(coffee, counters[coffee._id] || 1)}>Add
                              Coffee
                            </button>
                          </div>
                      ) : (
                          <>
                            <button className="delete-button"
                                    onClick={() => deleteCoffee(coffee._id, fetchCoffees)}>Delete
                            </button>
                            <button className="edit-button"
                                    onClick={() => setSelectedCoffee(coffee)}>Edit
                            </button>
                          </>
                      )}
                    </div>
                    <img src={favorite ? favoriteIcon : notFavoriteIcon} alt="Favorite" className="favorite-icon"
                         onClick={() => handleFavoriteToggle(coffee._id)}/>
                  </li>
              ))}
            </ul>
        )}
